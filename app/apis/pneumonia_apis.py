...
router = APIRouter(prefix="/api/v1/pneumonia", tags=["AI Pneumonia Prediction"])
...
async def predict_pneumonia_by_record(
    ...
    safe_copy_file(physical_path, heatmap_dir / f"record_{record_id}.png")
...
async def get_pneumonia_result_by_record(
    ...
    if not analysis_result:
        raise HTTPException(status_code=404, detail="해당 진료 기록에 대한 AI 분석 결과가 존재하지 않습니다.")
